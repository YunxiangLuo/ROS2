...
```bash
# ROS2 Humble + CARLA + ros-bridge( carla_ros_bridge )
pip install numpy
sudo apt install ros-humble-ros-bridge-carla-msgs-bridge  # 或源码编译 ros-bridge
```
...
`check_topics.py` 已修复 Windows GBK 控制台下输出 `勾/叉` 符号的
UnicodeEncodeError(添加 stdout UTF-8 兼容块)。
