...
```text
$ cd src && python -m pytest lab_code/tests/test_ch40_controllers.py -q
12 passed in 0.06s
```

脚本自带自检(--test, 修复 Windows 控制台编码缺陷后的真实输出):
...
`pid_controller.py`/`pure_pursuit.py`/`lat_lon_ctrl.py` 在 Windows GBK 控制台
输出 `✓/✗` 抛 UnicodeEncodeError → 添加 stdout UTF-8 reconfigure 兼容块。
