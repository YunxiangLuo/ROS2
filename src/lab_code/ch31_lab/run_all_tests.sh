...
print_banner() {
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║   ${WHITE}第45章 综合项目 测试套件${NC}"
...
check_dependencies() {
    ...
    if [ -n "${ROS_DISTRO:-}" ]; then
        ...
        echo -e "  ${GREEN}[OK]${NC} ROS2 humble (auto-loaded)"
    else
...
generate_report() {
    ...
    # 汇总结果
    {
        ...
        echo "<title>第45章 综合项目 - 测试报告</title>"
        echo "<style>
